...
import com.project.accountservice.models.response.CreditCardCreditCardResponse;
import com.project.accountservice.repository.IAccountRepository;
...
public class AccountServiceImpl implements IAccountService {
    ...
    /**
     * <p>Create method: An account is received with the necessary parameters, for later (1) the validation of the client,
     * following (2) the validation of the account, finally the account associated with the client is created.</p>
     *
     * @param account
     * @return Mono<AccountEntity>
     */
    @Override
        ...
        if (!account.getClientId().isBlank()) {
            Mono<AccountEntity> createdAccount = findByIdClientService(account.getClientId())
                    ...
                    .flatMap(validatedClient -> {
                        ...
                        AccountEntity accountToCreate = accountUtils.accountCreateRequestToAccount(account, client);
    ...
    public Mono<AccountFindBalancesResponse> findBalancesByAccountId(String numberAccount) {
        ...
        Mono<AccountFindBalancesResponse> retrievedBalances = findByAccountNumber(numberAccount)
                .flatMap(retrieveAccount ->{
                    AccountFindBalancesResponse accountFindBalancesResponse = accountUtils.accountToAccountFindBalancesResponse(retrieveAccount);
